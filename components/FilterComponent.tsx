...
const categories: Category[] = [
  ...
  },
  {
    ...
    options: [
      ...
      "Antenatal care",
      "Group consultation",
      "Adolescents & children consultation",
    ],
  },
      ...
      {categories.map(({ title, options, single }) => (
        <ThemedView style={{ paddingHorizontal: 15 }} key={title}>
          ...
          >
            ...
            {expanded[title] ? (
              <Entypo name="chevron-down" size={24} color="#0755D4" />
            ) : (
              <Entypo name="chevron-up" size={24} color="#0755D4" />
            )}
          </TouchableOpacity>
          <ThemedView>
        ...
        {[1, 2, 3, 4, 5].map((num) => (
          <ThemedView key={num} style={styles.starContainer}>
            ...
            <ThemedText
              style={[
                styles.star,
                { color: num <= rating ? "#FFD700" : "#C0C0C0" },
              ]}
            >
              ★
            </ThemedText>
